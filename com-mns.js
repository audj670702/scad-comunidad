...
function resolveMnsContext(ctx){const eoId=String(ctx?.mns?.eoId||ctx?.eo?.mnsEoId||ctx?.eo?.eoMnsId||'').trim();const eoKey=String(ctx?.eo?.codigoEO||ctx?.mns?.eoKey||ctx?.eo?.mnsEoKey||'').trim();if(!eoId&&!eoKey)throw new Error('La APP no recibió el EO MNS correspondiente a esta comunidad.');return eoId?{mnsKey:MNS_KEY,eoId}:{mnsKey:MNS_KEY,eoKey}}
