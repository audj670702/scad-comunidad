...
const MNS_EO_BY_COMMUNITY=Object.freeze({'BOYS SCOUTS':'EO-002'});
...
function resolveMnsContext(ctx){const community=String(ctx?.eo?.nombreVisible||ctx?.eo?.nombre||'').trim().toUpperCase();const eoId=String(ctx?.mns?.eoId||ctx?.eo?.mnsEoId||ctx?.eo?.eoMnsId||MNS_EO_BY_COMMUNITY[community]||'').trim();if(!eoId)throw new Error('La APP no recibió el EO MNS correspondiente a esta comunidad.');return {mnsKey:MNS_KEY,eoId}}
