...
function normalizeTokens(t){
  if(!t)return null;
  if(t?.accessToken?.value)return t;
  const accessValue=String(t.access_token||'').trim();
  const refreshValue=String(t.refresh_token||'').trim();
  if(!accessValue)return null;
  const expiresIn=Number(t.expires_in||3600);
  return{
    accessToken:{value:accessValue,expiresAt:Math.floor(Date.now()/1000)+expiresIn},
    refreshToken:refreshValue?{value:refreshValue}:undefined
  };
}
function readTokens(){try{return normalizeTokens(JSON.parse(localStorage.getItem(TOKEN_KEY)||'null'))}catch{return null}}
function saveTokens(t){const normalized=normalizeTokens(t);if(normalized)localStorage.setItem(TOKEN_KEY,JSON.stringify(normalized))}
function tokenExpired(accessToken){const expiresAt=Number(accessToken?.expiresAt||0);if(!expiresAt)return false;const expiresMs=expiresAt<1e12?expiresAt*1000:expiresAt;return Date.now()>=expiresMs-30000}
...
async function anonymousToken(){return normalizeTokens(await tokenRequest({clientId:CLIENT_ID,grantType:'anonymous'}))}
async function refreshTokens(refreshToken){const raw=await tokenRequest({clientId:CLIENT_ID,grantType:'refresh_token',refreshToken:refreshToken?.value||refreshToken});saveTokens(raw);return readTokens()}
async function accessToken(){let t=readTokens();if(!t?.accessToken?.value)return'';if(!tokenExpired(t.accessToken))return t.accessToken.value;if(t?.refreshToken?.value){t=await refreshTokens(t.refreshToken);return t?.accessToken?.value||''}return''}
...
async function startMnsLogin(){const verifier=randomString(72),state=randomString(32),codeChallenge=await challenge(verifier);sessionStorage.setItem(PKCE_KEY,JSON.stringify({verifier,state}));sessionStorage.setItem(PENDING_KEY,'1');const anon=await anonymousToken();const anonAccess=anon?.accessToken?.value||'';if(!anonAccess)throw new Error('No fue posible iniciar autenticación Wix.');const r=await fetch('https://www.wixapis.com/headless/v1/redirect-session',{method:'POST',headers:{'Content-Type':'application/json','Authorization':anonAccess},body:JSON.stringify({auth:{authRequest:{clientId:CLIENT_ID,responseType:'code',redirectUri:REDIRECT_URI,scope:'offline_access',state,responseMode:'query',codeChallenge,codeChallengeMethod:'S256',metaSiteId:SITE_ID},prompt:'login'},preferences:{useGenericWixPages:true}})});if(!r.ok)throw new Error(`OAuth redirect ${r.status}`);const data=await r.json();const url=data?.redirectSession?.fullUrl;if(!url)throw new Error('Wix no devolvió URL de autenticación.');location.assign(url)}
async function consumeCallback(){const p=new URLSearchParams(location.search),code=p.get('code'),error=p.get('error');if(error)throw new Error(`Autenticación Wix: ${error}`);if(!code)return false;const raw=sessionStorage.getItem(PKCE_KEY);if(!raw)return false;const pkce=JSON.parse(raw);if(p.get('state')!==pkce.state)throw new Error('Estado OAuth inválido.');const tokenResponse=await tokenRequest({clientId:CLIENT_ID,grantType:'authorization_code',redirectUri:REDIRECT_URI,code,codeVerifier:pkce.verifier});saveTokens(tokenResponse);sessionStorage.removeItem(PKCE_KEY);history.replaceState({},document.title,location.pathname);return true}
...
async function invokeMns(action,payload={}){const access=await accessToken();if(!access)throw new Error('Inicia sesión para usar Mensajería.');const tokens=readTokens();if(!tokens?.accessToken?.value)throw new Error('Inicia sesión para usar Mensajería.');const client=createClient({modules:{functions},auth:OAuthStrategy({clientId:CLIENT_ID,tokens})});const ctx=resolveMnsContext(activeContext);const response=await client.functions.post('mnsBridge',{headers:{'Content-Type':'application/json'},body:JSON.stringify({action,payload:{...payload,...ctx}})});const data=await response.json().catch(()=>({}));if(!response.ok)throw new Error(data?.error||`MNS no respondió (${response.status}).`);if(data?.ok!==true)throw new Error(data?.error||'No fue posible completar la operación.');return data.data}
