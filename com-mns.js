...
function resolveMnsContext(ctx){const eoId=String(ctx?.mns?.eoId||ctx?.eo?.mnsEoId||ctx?.eo?.eoMnsId||'').trim();if(!eoId)throw new Error('La APP no recibió el EO MNS correspondiente a esta comunidad.');return {mnsKey:MNS_KEY,eoId}}
