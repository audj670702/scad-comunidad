...
function resolveMnsContext(ctx){
  ...
  if(!eoId)throw new Error('La APP no recibió el EO MNS correspondiente a esta comunidad.');
  return {mnsKey:MNS_KEY,eoId};
}
...
async function invokeMns(action,payload={}){
  ...
  const client=createClient({modules:{functions},auth:OAuthStrategy({clientId:CLIENT_ID,tokens})});
...
async function openMns(ctx){
  activeContext=ctx||window.scadComContext?.()||null;
  if(!activeContext)return;
  if(!await accessToken()){await startMnsLogin();return}
  try{resolveMnsContext(activeContext)}catch(error){console.error('[SCaD COM MNS]',error);window.alert(error.message);return}
  ensureStyles();closeMns();
  const overlay=document.createElement('div');overlay.id='comMnsOverlay';overlay.className='com-mns-overlay';overlay.innerHTML=`<div class="com-mns-panel" role="dialog" aria-modal="true" aria-label="Mensajería"><iframe class="com-mns-frame" src="${FRAME_URL}" title="Mensajería SCaD MNS"></iframe></div>`;
  overlay.addEventListener('click',e=>{if(e.target===overlay)closeMns()});document.body.appendChild(overlay);document.body.classList.add('com-mns-open');
}
...
window.addEventListener('message',async event=>{
  ...
  if(m.type==='READY'){try{f.contentWindow.postMessage({channel:CHANNEL,type:'CONTEXT',payload:resolveMnsContext(activeContext)},location.origin)}catch(e){console.error('[SCaD COM MNS]',e)}return}
  if(!m.id||!m.action)return;
  try{const data=await invokeMns(m.action,m.payload||{});f.contentWindow.postMessage({channel:CHANNEL,id:m.id,ok:true,data},location.origin)}catch(error){f.contentWindow.postMessage({channel:CHANNEL,id:m.id,ok:false,error:error?.message||'Error MNS'},location.origin)}
});

(async()=>{try{const consumed=await consumeCallback();if(consumed&&sessionStorage.getItem(PENDING_KEY)==='1'){sessionStorage.removeItem(PENDING_KEY);let tries=0;const timer=setInterval(()=>{const ctx=window.scadComContext?.();if(ctx){clearInterval(timer);openMns(ctx)}else if(++tries>40)clearInterval(timer)},250)}}catch(e){console.error('[SCaD COM MNS OAuth]',e)}})();
