...
// SCaD COMUNIDAD · Integración MNS · v3.15
...
function resolveMnsContext(ctx){const community=String(ctx?.eo?.nombreVisible||ctx?.eo?.nombre||'').trim().toUpperCase();const eoId=String(ctx?.mns?.eoId||ctx?.eo?.mnsEoId||ctx?.eo?.eoMnsId||'').trim();const eoKey=String(ctx?.eo?.codigoEO||ctx?.mns?.eoKey||ctx?.eo?.mnsEoKey||MNS_EO_KEY_BY_COMMUNITY[community]||'').trim();if(!eoId&&!eoKey)throw new Error('La APP no recibió el EO MNS correspondiente a esta comunidad.');return eoId?{mnsKey:MNS_KEY,eoId}:{mnsKey:MNS_KEY,eoKey}}
